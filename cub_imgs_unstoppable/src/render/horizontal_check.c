...
void init_h_ray_up(t_data *d, t_ray *ray, t_ray_params *ray_params)
{
    ray_params->hy = floor(d->pl->y / TILE_SIZE) * TILE_SIZE - 0.0001;
    ray_params->hx = (d->pl->y - ray_params->hy) * ray_params->h_tan + d->pl->x;
    ray_params->yo = -TILE_SIZE;
    ray_params->xo = -ray_params->yo * ray_params->h_tan;    
}

void	init_h_ray_down(t_data *d, t_ray *ray, t_ray_params *params)
{
    ...
    while (ray_params->dof < d->map_rows)
    {
       ...
       map_y = (int)(ray_params->hy / TILE_SIZE);
        ...
        if (map_x >= 0 && map_x < d->map_cols && map_y >= 0 && map_y < d->map_rows && 
            ...
            ray_params->hx += ray_params->xo; // STEP_SIZE to next grid line
...
void horizontal_check(t_ray *ray, t_data *d, t_ray_params *ray_params)
{
    ...
    ray_params->dof = 0;
    if (ray->angle > M_PI)
        init_h_ray_up(d, ray, ray_params);
    else if (ray->angle < M_PI)
        init_h_ray_down(d, ray, ray_params);
    else
    {
